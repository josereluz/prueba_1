...
const POLIGONO_COLOR_PALETTE = [
  { line: "#e11d48", fill: "#e11d48" }, // rojo rosado
  { line: "#2563eb", fill: "#2563eb" }, // azul
  { line: "#f97316", fill: "#f97316" }, // naranja
  { line: "#7c3aed", fill: "#7c3aed" }, // violeta
  { line: "#db2777", fill: "#db2777" }, // fucsia
  { line: "#f59e0b", fill: "#f59e0b" }, // ambar
  ...
  { line: "#4338ca", fill: "#4338ca" }, // indigo oscuro
  { line: "#c2410c", fill: "#c2410c" }, // ladrillo
  { line: "#0369a1", fill: "#0369a1" }, // azul petroleo
  { line: "#9f1239", fill: "#9f1239" }, // vino
  { line: "#a16207", fill: "#a16207" }, // ocre
  { line: "#6d28d9", fill: "#6d28d9" }, // violeta oscuro
  { line: "#0ea5e9", fill: "#0ea5e9" }, // celeste intenso
  { line: "#d946ef", fill: "#d946ef" }, // fucsia brillante
  { line: "#f43f5e", fill: "#f43f5e" }, // rose
...
const POLIGONO_EDIT_STORAGE_KEY = "visor_comunicacion_poligono_ediciones_v1";
...
function formatDistrictDisplayName(name) {
  ...
  return raw.toLowerCase().replace(/(^|[\s\-\/])([^\s\-\/]+)/g, (m, sep, word) => {
    ...
    div.innerHTML = `
      <div class="dashboard-action-row">
        <div class="map-auth-wrap">
          <button type="button" id="map-auth-btn" class="map-auth-btn" aria-label="Login" title="Login">
            <span id="map-auth-dot" class="map-auth-dot" aria-hidden="true"></span>
            <span id="map-auth-text">Login</span>
          </button>
...
function updateLegend() {
  ...
  if (isOn("layer-poligonos-activos")) {
    rows.push(`<div class="legend-row">${legendItemSwatch("#f97316", "poly")}<span>Pol&iacute;gono CIC activos</span></div>`);
  } else if (isOn("layer-poligonos-supervision")) {
    rows.push(`<div class="legend-row">${legendItemSwatch(POLIGONOS_CIC_SOURCE.lineColor, "poly")}<span>Pol&iacute;gono</span></div>`);
  }
  ...
  if (isOn("layer-conflicto-zona")) rows.push(`<div class="legend-row">${legendItemSwatch("#dc2626", "poly")}<span>Zona en controversia</span></div>`);
...
makePane("sectorPane", 520);
makePane("manzanaPane", 580);
makePane("baseLabelPane", 590);
makePane("poligonoLabelPane", 610);
makePane("actividadPane", 606);
makePane("conflictPane", 594);
...
function syncAuthUi() {
  ...
  if (mapAuthBtn) {
    ...
    mapAuthBtn.setAttribute("title", logged ? "Usuario conectado" : "Login");
    mapAuthBtn.setAttribute("aria-label", logged ? "Usuario conectado" : "Login");
  }
  if (mapAuthText) mapAuthText.textContent = logged ? (currentSimpleUser.displayName || currentSimpleUser.username) : "Login";
...
async function submitPoligonoEditForm(ev) {
  ...
  try {
    ...
    refreshOpenPoligonoPopup();
    if (isPoligonoActivosModeEnabled()) {
...
function buildPoligonoPopup(props) {
  ...
  const rows = POLIGONO_POPUP_ROWS.map((row) => {
  ...
  return `
    <div class="popup-attrs popup-poligono">
      <h4>${escapeHtml(title)}</h4>
      <table>${rows || '<tr><td colspan="2">&mdash;</td></tr>'}</table>
      <div class="popup-poligono-actions">${editButton}</div>
      ${editedMeta}
    </div>
  `;
}
...
const layerPoligonosCic = L.geoJSON(null, {
  ...
  style: (ft) => {
    ...
    return {
      ...
      weight: activeOnly ? 2.8 : 2.2,
      opacity: 0.98,
      fillColor: activeOnly ? "#f97316" : c.fill,
      fillOpacity: activeOnly ? 0.075 : 0.032
    };
  },
  onEachFeature: (ft, lyr) => {
    ...
    lyr.bindPopup(
      () => buildPoligonoPopup(lyr.feature?.properties || props),
      { maxWidth: 340, className: "popup-poligono-wrapper" }
    );
    lyr.on("click", (ev) => {
      try { L.DomEvent.stop(ev); } catch (e) {}
      const clickedCode = getPoligonoValue(lyr.feature?.properties || props);
      if (clickedCode) flashPoligonoGroupByCode(clickedCode);
...
async function syncPoligonosCicLayerForUbigeos(ubigeos) {
  ...
  const activeOnly = isPoligonoActivosModeEnabled();
  const enabled = !!((checkbox && checkbox.checked) || activeOnly);
  ...
  try {
    const gj = await fetchGeoJSON(
      poligonosCicWfsUrl({ maxFeatures: 10000 }),
    ...
    const features = (Array.isArray(gj?.features) ? gj.features : []).filter((ft) => {
      ...
      const matchesDistrict = matchesUbigeo || POLIGONOS_CIC_SOURCE.districtFields.some((field) => districtNames.has(normalizeDistrictName(props?.[field])));
      if (!matchesDistrict) return false;
      return activeOnly ? isPoligonoCicActivo(props) : true;
    });
...
map.on("zoomend moveend resize", () => { updateLabelOpacity(); syncPoligonoLabels(); });
updateLabelOpacity();
...
function showOnlyPoligonoFeatures(features, opts = {}) {
  ...
  const activos = document.getElementById("layer-poligonos-activos");
...
async function searchPoligonoWfs(poligonoText, activeKeys) {
  ...
  const gj = await fetchGeoJSON(poligonosCicWfsUrl({ maxFeatures: 10000 }), { ttlMs: CACHE.queryTtlMs });
  const allFeatures = (Array.isArray(gj?.features) ? gj.features : []).filter((ft) => {
    const props = ft?.properties || {};
    return poligonoCodeMatches(getPoligonoValue(props), terms);
  });
...
async function doSearch() {
  ...
  if (!poligono && !activeKeys.length && !sector && !mz) {
    if (searchResult) searchResult.textContent = "Aviso: selecciona Distrito/Lote o ingresa Poligono, Sector o Manzana.";
    return;
  }
  if (lt && !mz) {
...
bindToggle(
  ...
  async () => {
    ...
    const keys = getActiveUbigeos();
    if (keys.length) await syncPoligonosCicLayerForUbigeos(keys);
    else if (!map.hasLayer(layerPoligonosCic)) map.addLayer(layerPoligonosCic);
    syncPoligonoLabels();
    updateLegend();
    ensureLayerOrder();
  },
...
bindToggle(
  ...
  async () => {
    const poligonoCheckbox = document.getElementById("layer-poligonos-supervision");
    if (poligonoCheckbox) poligonoCheckbox.checked = true;
    _activePoligonoSearch = null;
    const keys = getActiveUbigeos();
    if (keys.length) await syncPoligonosCicLayerForUbigeos(keys);
    syncPoligonoLabels();
  ...
  async () => {
    const keys = getActiveUbigeos();
    const poligonoCheckbox = document.getElementById("layer-poligonos-supervision");
    if (poligonoCheckbox && poligonoCheckbox.checked && keys.length) {
      await syncPoligonosCicLayerForUbigeos(keys);
    } else if (!poligonoCheckbox || !poligonoCheckbox.checked) {
      layerPoligonosCic.clearLayers();
      if (map.hasLayer(layerPoligonosCic)) map.removeLayer(layerPoligonosCic);
    }
    syncPoligonoLabels();
...
bindToggle(
  ...
  async () => {
    const poligonoCheckbox = document.getElementById("layer-poligonos-supervision");
    if (poligonoCheckbox && !poligonoCheckbox.checked) {
      poligonoCheckbox.checked = true;
      const keys = getActiveUbigeos();
      if (_activePoligonoSearch && Array.isArray(_activePoligonoSearch.features) && _activePoligonoSearch.features.length) {
        showOnlyPoligonoFeatures(_activePoligonoSearch.features, { fit: false });
      } else if (keys.length) {
