...
const CACHE = {
  enabled: true,
  version: "v11-base-wfs-vector-no-popup-label-inside",
  memoryMax: 25,
...
async function fetchBaseLayerByUbigeo(typeName, ubigeo, signal) {
  ...
  for (const field of getUbigeoFieldCandidates()) {
    const cql = _cqlByField(field, ubigeo);
    try {
...
async function loadBaseForUbigeo(ubigeo) {
  ...
  try {
    ...
    const [gjS, gjM] = await Promise.all([
      fetchBaseLayerByUbigeo(GEO.layers.sector, requested, _baseAbort.signal),
      fetchBaseLayerByUbigeo(GEO.layers.manzana, requested, _baseAbort.signal)
    ]);
...
// Se mantiene el popup únicamente para capas operativas como Polígonos CIC.
...
map.on("zoomend moveend resize", updateLabelOpacity);
updateLabelOpacity();
...
async function doSearch() {
  ...
  try {
    ...
    if (lt) {
      if (searchResult) searchResult.textContent = "La capa base de Lote fue reemplazada por Sector. Para ubicar predios, usa Distrito, Sector y/o Manzana.";
      return;
    }

    if (!hasBaseMz) {
      if (searchResult) searchResult.textContent = "Cargando datos de Manzana... intenta de nuevo en unos segundos.";
      return;
    }
    ...
    (_baseFeatureIndex.manzana || []).forEach((item) => {
      ...
      if (ubigeo && !eqCode(getFeatureValue(p, "ubigeo"), ubigeo)) return;
